...
\item $\aK = \grec[@][@][@][\aK']$ $(\atuple, \aloc)$ is local to $\aK'$
\end{itemize}
%
...
functions:
\begin{align*}
  \mathit{STOP}(\arole, \aphi) = & \apref[\arole][\arole_1][{\asort[stop]}][\aphi(\arole_1)] \seqop \ldots \seqop \apref[\arole][\arole_n][{\asort[stop]}][\aphi(\arole_n)]
  \\
  \mathit{LOOP}(\arole, \aphi, \aK) = & \apref[\arole][\arole_1][{\nu y_1:\asort[loc]}][\aphi(\arole_1)] \seqop \ldots \seqop \apref[\arole][\arole_n][{\nu y_n:\asort[loc]}][\aphi(\arole_n)] \seqop \aK
\end{align*}
where $\dom \aphi = \{\arole_1, \ldots, \arole_n\}$ and
$\aphi : \arole_i \mapsto \aloc_i$ for $i \in \{1, \ldots, n\}$.
%
Then, we define
\begin{align*}
  \ksem{\grec} =
  &
    \begin{cases}
      \bigcup_{h \geq 0} \ksem{\unfold h \grec} & \text{if } \ws[\aK\sust X \nil][\aK\sust X \nil]
      \\
...
\[
  \unfold h \grec =
  \begin{cases}
    \mathit{STOP}(\arole,\aphi) & \text{if } h = 0
    \\
    \mathit{LOOP}(\arole,\aphi,\aK) \seqop \unfold{h-1}{\grec} & \text{otherwise}
  \end{cases}
\]
%
