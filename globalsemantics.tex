...
To capture this semantics we label events with (decorations of)
autonomous prefixes $\apref$: labels are either just autonomous
prefixes $\apref$ or of the form $\alabel$ with $\apref$ an autonomous
prefix.
\todo{eM: qui}
...
$\bp = \left\{ \pomsetrep{\node {$\alabel$}}[] \right\}$.
%
For events emerging from interactions labels have to be \quo{marked}
...
\begin{align*}
  ...
  &
    ...
    \right\}
\end{align*}
where $\atuple'$ is obtained by removing $\nu$-bound names with
$\wildcard$ in $\atuple$.
...
Let us start with well-sequencedness.
%
To explain the peculiarities of well-sequencedness, let us consider
...
tuple types: a choreography $\aK_1 \seqop \aK_2$ is
...
\item for all $(\atuple,\aloc)$ in a consuming-input prefix of $\aK_i$
  and non-local in $\aK_i$ and for all $(\atuple', \aloc)$ in an
  output prefix of $\aK_j$, $\atuple \matches \atuple'$ implies
  $(\atuple',\aloc)$ is in a consuming output prefix in $\aK_j$.
\end{itemize}
...
composes pomsets.
%
The sequential composition of $\apom$ and $\apom'$ has to order the
events triggered by a some instance and those corresponding to
autonomous outputs in $\apom$ with those corresponding to autonomous
accesses in $\apom'$.
...
$\esetof \apom \cap \eset \apom' = \emptyset$,
recall that the
