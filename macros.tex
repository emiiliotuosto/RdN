...
\DeclareMathOperator{\seqop}{\mkop{;}}
\newcommand{\rec}{\mkop{\mathtt{rec}}\xspace}
\newcommand{\nil}{\mkop{\textbf{0}}}
...
\newcommandx{\proj}[3][1=\aK,2=\arole,3=\eta,usedefault=@]{
  \ifempty{#1}{\_}{#1} \downharpoonleft_{\ifempty{#2}{\_}{#2}}^{\ifempty{#3}{\_}{#3}}
}
...
%}
\newcommandx{\arec}[4][1=X,2=\aK,3={\arole},4={\aphi},usedefault=@]{
  {\rec_{#3}^{#4}\ #1 \mkop{.} #2}
}
\newcommand{\aphi}{\phi}
...
\newcommandx{\rseq}[2][1=\apom,2={\aapom'},usedefault=@]{\gfun{seq}({#1},{#2})}
...
\newcommand{\unfold}[2]{\text{unfold}_{#1}(\ifempty{#2}{\_}{#2})}
...
{\big(\rec_{#5}#1({#2}) \mkop{.} #3}\big)\langle#4\rangle}

\newcommandx{\abasiclrec}[3][1=X,2=\seq x,3=\aL,usedefault=@]{
{\big(\rec #1({#2}) \mkop{.} #3}\big)}
